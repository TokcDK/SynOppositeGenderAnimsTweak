...
namespace SynOppositeGenderAnimsTweak
{
    public class Program
    {
        ...
        public static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
            ...
            var excludedListExt = PatchSettings.Value.ExcludedNPCKeywordsListExtended;
            bool isUseExcludedNpcListExt = excludedListExt.Count > 0 && excludedListExt.HasAnyValidValue();
            ...
            foreach (var npcGetter in state.LoadOrder.PriorityOrder.Npc().WinningOverrides())
            {
                ...
                // check by skip lists
                if (!string.IsNullOrWhiteSpace(npcGetter.EditorID))
                {
                    if (isUseExcluded && isUseExcludedNpcList
                        && excludedList.Contains(npcGetter.EditorID)) continue; // editor id contains one of keywords from exclusions
                    if (isUseExcludedNpcListExt && npcGetter.EditorID.HasAnyFromList(excludedListExt)) continue; // check by extended list
                }
